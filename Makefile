...
.PHONY: help resources index preprocess build up down logs smoke clean \
	build-rust up-rust down-rust logs-rust smoke-rust ship-rust
...
help:
	...
	@echo
	@echo "Rust targets (uses docker-compose.rust.yml):"
	@echo "  build-rust  — docker compose -f docker-compose.rust.yml build"
	@echo "  up-rust     — docker compose -f docker-compose.rust.yml up -d --build"
	@echo "  down-rust   — docker compose -f docker-compose.rust.yml down"
	@echo "  logs-rust   — docker compose -f docker-compose.rust.yml logs -f"
	@echo "  smoke-rust  — curl smoke against http://localhost:9999 (Rust stack)"
	@echo "  ship-rust   — build, tag, and push caiogallo2401/sketchy-rust:\$$VERSION"
...
logs-rust:
	docker compose -f docker-compose.rust.yml logs -f --tail=200
...
ship-rust: resources
	...
	docker compose -f docker-compose.rust.yml build api1
	docker tag sketchy-rust:local caiogallo2401/sketchy-rust:$(VERSION)
	docker push caiogallo2401/sketchy-rust:$(VERSION)
	@echo
	@echo "Pushed caiogallo2401/sketchy-rust:$(VERSION)."
	@echo "If submitting under a separate id, register sketchy-rust in"
	@echo "participants/CaioDGallo.json and open issue body 'rinha/test sketchy-rust'."
